...
- Put new audio assets in the narrowest existing `Resources/Audio` subtree; use `_SD` for SD-only assets and the existing inherited/vendor subtree for features that already own one.
