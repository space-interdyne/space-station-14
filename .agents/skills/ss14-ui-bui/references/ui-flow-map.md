...
The fork keeps SD-specific client UI under `_SD` (for example `Input` and `Options`). If a new SD-specific UI is required, prefer either:

- extending the existing domain folder when the feature belongs to a normal SS14 system, or
- adding a clearly named `_SD` domain only when the behavior is genuinely fork-specific
