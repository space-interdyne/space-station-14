...
This repository is Space Dream (SD), a Space Station 14 fork with a clear split between gameplay code, client code, and content data:
...
- For SD-only behavior, prefer existing `_SD` folders when they exist. This repository also contains inherited/vendor-specific trees such as `_DV`, `Corvax`, `Nyanotrasen`, and others; extend the tree that already owns the feature instead of assuming all fork code belongs under `_SD`.
- When you must touch an upstream content file, keep the diff narrow and preserve surrounding structure and style.
- When adding or changing SD-specific code in an inherited file outside any `_SD` path, mark it when the surrounding file already uses edit markers or the change would otherwise be hard to distinguish:
...
- Keep edit-marker ranges as narrow as practical. For files that do not use `//` comments, use the native comment syntax while preserving `SD-Edit-Start`, `SD-Edit-End`, and `SD`.

## Assembly Placement

- Put shared data, shared events, networked state, and predicted logic in `Content.Shared/` (including `Content.Shared/_SD/` for fork-local shared code).
- Put server-only authority and non-predicted server simulation in `Content.Server/` (including `Content.Server/_SD/` for fork-local server code).
- Put client-only visuals, overlays, XAML, and BUI front-ends in `Content.Client/` (including `Content.Client/_SD/` for fork-local client code).
...
- Add or update FTL entries under `Resources/Locale/`, starting with `en-US`; add/update matching `ru-RU` entries when the same feature already maintains Russian locale or the change is SD-facing and you can do so without guessing translations.
- Use specific `kebab-case` localization IDs.
