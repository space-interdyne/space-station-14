...
Keep authority, persistence, and round rules here. Put SD-only server systems under `Content.Server/_SD/`. If the player should feel the effect immediately, verify the shared prediction path instead of leaving the feature server-only by accident.
