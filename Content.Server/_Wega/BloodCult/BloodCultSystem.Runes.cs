...
public sealed partial class BloodCultSystem
{
    ...
    private bool IsSyntheticBody(EntityUid target)
    {
        if (!TryComp<HumanoidProfileComponent>(target, out var profile))
            return false;

        return profile.Species == "Android" || profile.Species == "Ipc";
    }
    ...
    private void ApplyBloodboilDamage(MapCoordinates coords, EntityUid cultist, int damageValue)
    {
        ...
        {
            if (HasComp<BibleUserComponent>(targetFlammable.Owner))
                continue;
            ...
            _damage.TryChangeDamage(targetFlammable.Owner, damage, false);
        }

        foreach (var channelingCultist in _entityLookup.GetEntitiesInRange<BloodCultistComponent>(coords, 2f))
        {
            if (_mobState.IsDead(channelingCultist.Owner))
                continue;

            var selfDamage = new DamageSpecifier { DamageDict = { { "Heat", damageValue } } };
            _damage.TryChangeDamage(channelingCultist.Owner, selfDamage, false);
        }
    }
    ...
    private void EnterSpiritRealm(EntityUid cultist, MapCoordinates coords, EntityUid mindId)
    {
        ...
        _ghost.SetCanReturnToBody(ghost, canReturn);
    }
