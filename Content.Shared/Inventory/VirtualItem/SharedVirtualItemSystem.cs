...
public abstract partial class SharedVirtualItemSystem : EntitySystem
{
    ...
    public bool TrySpawnVirtualItem(EntityUid blockingEnt, EntityUid user, [NotNullWhen(true)] out EntityUid? virtualItem)
    {
        ...
        var virtualItemComp = EnsureComp<VirtualItemComponent>(virtualItem.Value); // SD
    ...
    {
        var userEv = new VirtualItemDeletedEvent(item.Comp.BlockingEntity, user, item.Owner); // SD
        RaiseLocalEvent(user, userEv);

        var targEv = new VirtualItemDeletedEvent(item.Comp.BlockingEntity, user, item.Owner); // SD
