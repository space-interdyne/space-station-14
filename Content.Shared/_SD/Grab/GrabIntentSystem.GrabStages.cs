...
public sealed partial class GrabIntentSystem
{
    ...
    private void OnAttacked(EntityUid uid, GrabIntentComponent component, AttackedEvent args)
    {
        ...
        float escapeAttemptModifier = 1f)
    {
        ...
        if (raiseEv.NewStage != null)
            newStage = raiseEv.NewStage.Value;
        ...
        bool dirty = true)
    {
        ...
        var massMultiplier = Math.Clamp(GrabMassHelper.MassRatio(EntityManager, pullable.Owner, puller.Owner) * 2f, 0.5f, 2f);
