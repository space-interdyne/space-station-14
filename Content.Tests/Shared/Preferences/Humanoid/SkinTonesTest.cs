...
public sealed class SkinTonesTest
{
    ...
    // SD edit start
    /// <summary>
    /// Checks that colors generated by FromUnary remain valid after byte quantization,
    /// matching the ClampColor path used by HumanoidCharacterAppearance.
    /// </summary>
    [Test]
    public void TestHumanSkinTonesFromUnarySurviveByteClamp()
    {
        var strategy = new HumanTonedSkinColoration();
        ISkinColorationStrategy verification = strategy;

        for (var i = 0; i <= 10000; i++)
        {
            var unaryInput = i / 100f;
            var color = HumanoidCharacterAppearance.ClampColor(strategy.FromUnary(unaryInput));
            Assert.That(verification.VerifyClampedSkinColor(color, out var reason),
                $"Color {color} from unary value {unaryInput} failed verification after byte clamp. Reason: {reason}");
        }
    }
    // SD edit end
